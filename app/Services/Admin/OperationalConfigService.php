...
{
    private const GROUP_ID = 'operational_config';
    ...
    public function resolveEditableRule(Setting $setting): Setting
    {
        if (strtolower((string) $setting->group_id) !== self::GROUP_ID) {
            throw new RuntimeException('Konfigurasi ini bukan termasuk grup aturan aplikasi.');
        }
    ...
    private function assertCodeUniqueness(string $code): void
    {
        ...
        if ($exists) {
            throw new RuntimeException('Kode konfigurasi sudah digunakan.');
        }
    }
